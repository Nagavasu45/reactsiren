...
const Detailcompo = () => {
    ...
    // }))
    // // console.log(cat1)
  const individual=data1.find((data2)=>{return data2.id===parseInt(id)})
  console.log(individual)
  return (
    <>
      <div className='line1'></div>
        {/* {data1.filter((data2)=>{return data2.id= }).map((item,index)=>{             */}
            {/* return( */}
                           
                <div >
                 
                  <div className='d1'><h4>{individual.heading}</h4></div>
                  </div>
                  <div className='set1'>
                ...
                <img src={individual.imgstore} alt='not loaded' style={{width:'340px', height:'400px'}}/>
                <div className='d1'><p>{individual.details}</p>
                <p>{individual.indetaildata}</p>
                </div>
                ...
                {data1.filter((item1)=>{ return item1.catageory===catageory}).slice(0,3).map((item,index)=>{
                  return(
                    
                    <div className='last2' div key={index}>
